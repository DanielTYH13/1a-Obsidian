...
*Key Concepts:*
___
- [[Archive/MATH115/2/2.1/Systems of Linear Equations]] can be represented as a ==matrix-vector== product.
...
- Note: With the matrix $A\vec{x} = \vec{b}$ where $A \in M_{mxn}(\mathbb{R})$, $\vec{b}\in \mathbb{R}^m$
	- This is because $\vec{b}$ is the matrix-vector product, which is dependant on m ([[Archive/MATH115/3/3.2/Matrix-Vector Multiplication]]).
